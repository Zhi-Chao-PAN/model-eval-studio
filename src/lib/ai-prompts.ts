...
请根据用户要求修改题目。保持原有的两部分结构（任务 Prompt + 题目来源/背景说明），用中文输出。`.trim()
}
...
【产物效果反馈】
（必须结合产物内容和”真实产物验证证据解读”来写，以第一人称”我”描述自己核验产物后的真实反馈。只陈述截图确实展示的界面、文本、数据或运行结果；后台自动截图可以证明产物内容已被系统打开查看，但不得把受控文件预览说成工具已执行，只有明确的沙箱核验日志才能支撑”已运行工具”的表述。如果没有真实截图，明确指出未提供真实产物验证截图，反馈只能基于上传产物文本和任务要求判断。反馈要具体：是否符合预期、有无错误遗漏、可用性如何。）
