...
【产物核验证据解读】
${opts.verificationSummary || '未提供产物核验证据，反馈只能基于上传产物文本和任务要求判断。'}
...
【产物效果反馈】
（必须结合产物内容和“产物核验证据解读”来写，以第一人称“我”描述核验产物后的真实反馈。只陈述截图确实展示的界面、文本、数据或运行结果；后台代验截图可以证明产物内容已被系统打开查看，但不得把受控文件预览说成工具已执行，只有明确的沙箱核验日志才能支撑“已运行工具”的表述。如果没有核验证据，明确写“未提供产物核验证据”，反馈只能基于上传产物文本和任务要求判断。反馈要具体：是否符合预期、有无错误遗漏、可用性如何。）

【模型交付效率是否符合预期？】
评分：x.x / 10
评论：（交付效率评分允许 1-10，支持 .5，例如 6.5、7.5。${isCoding ? '重点结合轨迹质量：步骤可复现性、验证闭环、工具/命令使用效率。' : isAgent ? '重点结合规划能力、工具调用效率、是否陷入死循环或反复无效尝试。' : '重点结合耗时、轮次、工具调用、重试、绕路、是否一次交付到位。'}）

【模型的产物质量怎么样】
评分：x.x / 10
评论：（产物质量评分允许 1-10，支持 .5，例如 6.5、7.5。${isCoding ? '重点结合需求完成度和代码质量：正确性、完整性、结构清晰度、可维护性、有无安全风险。' : isAgent ? '重点结合交付结果质量和指令遵循度：是否准确理解意图、交付物是否符合要求、质量如何。' : '重点结合正确性、完整性、可用性、专业性、可读性、工程质量。'}）

【模型的综合表现怎么样】
评分：x / 10
评论：（综合评分必须是 1-10 的整数，不允许 .5。结合产物效果、交付效率、产物质量、真实需求契合度给出整体评价。${isCoding ? '注意封顶规则：核心需求未完成不超过6，不可运行不超过4，明显跑题不超过2。' : ''}）
...
【产物核验证据解读】
${opts.verificationSummary || '未提供产物核验证据。'}
